  ...
  describe("UI-06: Comments vs Notes visual distinction & role isolation (AC-12, AC-13, AC-04)", () => {
    ...
    it("strictly omits Internal Notes from the DOM when viewed by a Requester (AC-04)", async () => {
      render(
        <TicketDetail
          ticketId="ticket-uuid-123"
          currentRequester={mockRequesterUser}
          onBack={vi.fn()}
        />
      );
    ...
    it("allows posting a new public comment", async () => {
      ...
      await waitFor(() => {
        expect(api.createTicketComment).toHaveBeenCalledWith(
          "ticket-uuid-123",
          "I rebooted the router as requested."
        );
      });
    ...
    it("allows IT Staff to post an internal note", async () => {
      ...
      await waitFor(() => {
        expect(api.createTicketNote).toHaveBeenCalledWith(
          "ticket-uuid-123",
          "Followed up with network operations center."
        );
      });
    ...
    it("renders Problem Appears Resolved button for Requester and handles click", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/you indicated this issue appears resolved/i)
        ).toBeInTheDocument();
      });
    });
  });
